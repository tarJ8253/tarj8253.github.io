...
lang: ja
#2019.9.2
#+title: EmacsのScratchBufferで計算
#*COMMENT * Emacsスクラッチバッファで計算
---
...
1. 変数名やカッコの後ろでC-j(Controlキーとjを同時に押す)と、変数名や
   カッコ内が評価される。
2. カッコの中に数式を書くと演算ができる。
3. 除算する時は数値は実数(x.0)と書く。いわゆる整数演算すると小数点以
   下が得られません。
4. 演算子は最初に書く。
