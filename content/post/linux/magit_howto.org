...
3. gitで管理するファイルを選択する
   ...
   が表記される
    ...
    カーソルをファイル名に移してから、sキーを押すと、そのファイルはステージに上がる。@@html:<br>@@
    S(大文字S)なら全ファイル指定。@@html:<br>@@
    C-u-i ならignoreファイルとして指定
    
    ?から始めても良い。コマンド一覧が表示

4. c@@html:<br>@@
   同じ画面からcを押すとArguments/Create/EditHEADなどの画面になる。@@html:<br>@@
   もう一度cを押してCommit注釈を入力画面にする。
   commitの注釈を入力する。@@html:<br>@@
   注釈文を書かないと、commitできない。 @@html:<br>@@

   注釈文は1行目に主な項目、3行目以降に詳細を記述。
   2行目には書かないこと(赤字になる)@@html:<br>@@
...
6. pushするには、P@@html:<br>@@
   push先のurl登録してなければ、聞いてくる。@@html:<br>@@
