...
2.要素への代入は(代入演算子は@@html:<font color="red">@@ : @@html:</font>@@)
...
@@html:<font color='red'>@@ * @@html:</font>@@(アスタリスク)を使うと、要素毎の積になるので注意。
...
行列式:determinant(C)
固有値:eigenvalues(D)
固有ベクトル:eigenvectors(E);
#+end_src

もし実行できなければ、各種パッケージを読み込んでください。
