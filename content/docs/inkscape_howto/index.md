...
inkscapeで作成する図形は、「図形の周囲(境界)」と「図形の内部」に分かれ
ています。
...
shiftキーを押しながら好きな色のパレットをクリックすると、ストローク(線)の色が設定されます。

![](./figs/line/line-color.png)

その後の微調整はColor-Wheelからできます。直線(折れ線)が現れました。

![](./figs/line/stroke-color.png)

オブジェクト選択後に、
好きな色のパレットをクリックすると、フィル(周辺、境界内部)の色が設定さ
れます。
先程は、フィル/ストロークともに黒色だったため、ヘンに見えただけです。
...
- グループ選択したり、解除したりして、編集の効率を上げます。

例えば、図形の中に文字を入れて一体として移動したいとき、グループ選択し
ます。
